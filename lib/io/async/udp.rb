    ...
    class UDP
      ...
      def initialize(fd:, state: :open, error_policy: nil)
        ...
        @context = if :open == state
          Internal::States::Socket::Open.new(fd: fd, backend: Internal::Backend::Async, parent: self)
        elsif :connected == state
          Internal::States::Socket::Connected.new(fd: fd, backend: Internal::Backend::Async)
        else
          Internal::States::Socket::Closed.new(fd: -1, backend: Internal::Backend::Async)
        end
      end
